...
import {
  ...
  widgetConfigureRowHeightRatios,
  widgetConfigureSlotInsetXRatio,
  widgetConfigureSlotInsetYRatio,
  widgetConfigureSlotCornerRadiusRatio,
} from '../design-system';
import { panelLayout } from '../ui/ultra/panelLayout';
import {
...
export function widgetConfigureSlotCornerRadius(height: number) {
  return height * widgetConfigureSlotCornerRadiusRatio;
}
...
export function widgetConfigureCellMetrics(widgetSize: number): WidgetConfigureCellMetrics {
  ...
  const rowHeights = widgetConfigureRowHeightRatios.map((ratio) => innerHeight * ratio);
  let rowTop = 0;
...
function widgetConfigureButtonFrame(cellWidth: number, cellHeight: number) {
  const insetX = cellWidth * widgetConfigureSlotInsetXRatio;
  const insetY = cellHeight * widgetConfigureSlotInsetYRatio;
